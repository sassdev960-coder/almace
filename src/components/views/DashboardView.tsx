...
import { InsumoItem, FormulaItem, DepartamentoItem, LoteItem, ViewType, MovimientoFinanzaItem } from '../../types';
import { Layers, FlaskConical, Box, Building2, PlayCircle, Plus, CheckCircle2, Clock, ChevronRight, AlertTriangle, DollarSign, ArrowUpRight, Activity, FileText, Wallet } from 'lucide-react';
...
interface DashboardViewProps {
  ...
  lotes: LoteItem[];
  finanzas?: MovimientoFinanzaItem[];
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // Calculate Net Cash balance from Finanzas
  const saldoNetoCaja = useMemo(() => {
    if (!finanzas || finanzas.length === 0) return 0;
    return finanzas.reduce((acc, m) => {
      const val = Number(m.monto) || 0;
      return m.tipo === 'ingreso' ? acc + val : acc - val;
    }, 0);
  }, [finanzas]);
  ...
  const stats = [
    ...
    {
      id: 'caja-flujo',
      label: 'Saldo Caja Operativa',
      value: `${saldoNetoCaja >= 0 ? '+' : ''}${saldoNetoCaja.toLocaleString('es-CO', { maximumFractionDigits: 0 })} $`,
      icon: Wallet,
      color: saldoNetoCaja >= 0 ? 'from-emerald-500/20 to-teal-500/20 text-emerald-400 border-emerald-500/30' : 'from-rose-500/20 to-amber-500/20 text-rose-400 border-rose-500/30',
      view: 'finanzas' as ViewType,
      sublabel: `${finanzas.length} ingresos y egresos registrados`,
      isCurrency: true,
      highlight: true,
      badge: 'Caja & Gastos',
    },
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          {onOpenTutorial && (
            <button
              ...
              id="dashboard-btn-tutorial"
              className="px-3.5 py-2.5 bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 border border-amber-500/30 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm active:scale-95"
              title="Ver guía y tutorial paso a paso"
            >
