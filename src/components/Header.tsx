...
import { RefreshCw, LogOut, FileText, Share2, HelpCircle } from 'lucide-react';
...
const VIEW_TITLES: Record<ViewType, { title: string; subtitle: string }> = {
  ...
  finanzas: { title: 'Caja & Finanzas', subtitle: 'Control de caja, egresos operativos y servicios' },
};
        ...
        <div className="flex items-center gap-2">
          {onOpenTutorial && (
            <button
              ...
              title="Guía y Tutorial de Uso"
              id="header-btn-tutorial"
              className="p-2 sm:px-3 sm:py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer active:scale-95 shadow-sm"
            >
              <HelpCircle className="w-3.5 h-3.5 text-amber-400" />
              <span className="hidden sm:inline">Tutorial / Guía</span>
            </button>
          )}
